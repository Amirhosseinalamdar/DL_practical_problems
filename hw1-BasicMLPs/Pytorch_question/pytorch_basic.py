...
def create_sample_tensor():
  ...
  # x = torch.zeros(size = (3, 2), dtype = int)
  # x[0, 1] = 10
  # x[1, 0] = 100
...
def make_one_hot(x):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  y = torch.zeros((len(x), max(x) + 1), dtype = int)
  y[range(len(x)), x] = 1
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # y = x.unsqueeze(-1).transpose(0, 1).reshape((3, 8))
  # pass
  z = x.view(2, 3, 4)
  y = torch.hstack([z[0], z[1]])
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  z = x.clone()
  mean = z.sum(dim = 0) / z.shape[0]
  var = ((z - mean) ** 2).sum(dim = 0) / (z.shape[0] - 1)
  sd = torch.sqrt(var)
  y = (z - mean) / sd 
  #############################################################################
  #                            END OF YOUR CODE                               #
